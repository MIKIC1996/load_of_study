...
void sdy_i2c_wait_ee_standby(){
	...
	do{
		...
		I2C_Send7bitAddress(SDY_I2C,SDY_I2C_EE_ADDR_BLOCK0,I2C_Direction_Transmitter);
...
///写入一页的数据
uint8_t sdy_i2c_ee_writePage(const u8* data, u8 size ,u8 memAddr){
	assert_param(size <= 8);
	uint32_t timeout = SDY_I2C_LONG_TIMEOUT;
	///若I2C忙碌等待
...
///优化过的 对齐写入函数
uint8_t sdy_i2c_ee_writeBuffer(const u8* data, u8 size ,u8 memAddr){
	
	///计算在单页中的地址
	u8 pageAddr = memAddr % SDY_I2C_EE_PAGE_SIZE;
	///补满第一页，需要的个数
	u8 firstPageFillCount = SDY_I2C_EE_PAGE_SIZE - pageAddr;
	...
	while(size > 0){
		...
		sdy_i2c_ee_writePage(data,memAddr,curWriteSize);
